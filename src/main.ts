...
import { DEFAULT_SETTINGS, PDFPlusSettingTab, PDFPlusSettings } from "settings";
import {
	DestArray,
	ObsidianViewer,
	PDFEmbed,
...
import { LibraryView } from "LibraryManagement/LibraryView";
...
export default class LibraryPlugin extends Plugin {
	...
	async onload() {
		...
		this.registerObsidianProtocolHandler(
			"pdf-plus",
			this.obsidianProtocolHandler.bind(this)
		);
	...
	checkVersion() {
		...
		if (requireApiVersion(untestedVersion)) {
			console.warn(
				`${this.manifest.name}: This plugin has not been tested on Obsidian ${untestedVersion} or above. Please report any issue you encounter on GitHub (https://github.com/RyotaUshio/obsidian-pdf-plus/issues/new/choose).`
			);
		}
	...
	async loadSettings() {
		...
		if (!this.settings.anystylePath) {
			const anystylePathFromLocalStorage =
				this.loadLocalStorage("anystylePath");
		...
		if (
			...
			this.settings.defaultDisplayTextFormatIndex >=
				this.settings.displayTextFormats.length
		) {
			this.settings.defaultDisplayTextFormatIndex = 0;
		}
		if (
			this.settings.defaultColorPaletteActionIndex < 0 ||
			this.settings.defaultColorPaletteActionIndex >=
				this.settings.copyCommands.length
		) {
			this.settings.defaultColorPaletteActionIndex = 0;
		}
		...
		if (
			(this.settings.paneTypeForFirstMDLeaf as PaneType | "") === "split"
		) {
		...
		if (this.settings.hasOwnProperty("showCopyLinkToSearchInContextMenu")) {
			...
			if (searchSectionConfig) {
				// @ts-ignore
				searchSectionConfig.visible &&=
					this.settings.showCopyLinkToSearchInContextMenu;
			}
			// @ts-ignore
		...
		this.renameSetting(
			"selectToCopyToggleRibbonIcon",
			"autoCopyToggleRibbonIcon"
		);
		this.renameCommand(
			"pdf-plus:toggle-select-to-copy",
			`${this.manifest.id}:toggle-auto-copy`
		);

		this.renameSetting(
			"removeWhitespaceBetweenCJKChars",
			"removeWhitespaceBetweenCJChars"
		);
	...
	private registerRibbonIcons() {
		// Add Library ribbon icon
		const ribbonIconEl = this.addRibbonIcon(LibraryIcon, "Open Library", () => {
			this.activateView();
		});
		ribbonIconEl.addClass("library-ribbon-icon");
		...
		if (this.settings.autoFocusToggleRibbonIcon) {
			...
			this.autoFocusToggleIconEl.toggleClass(
				"is-active",
				this.settings.autoFocus
			);

			this.registerDomEvent(
				this.autoFocusToggleIconEl,
				"contextmenu",
				(evt) => {
					if (menuShown) return;
					...
					menu.addItem((item) => {
						item.setIcon("lucide-settings")
							.setTitle("Customize...")
							.onClick(() => {
								this.openSettingTab().scrollToHeading(
									"auto-focus"
								);
							});
					});
		...
		if (this.settings.autoPasteToggleRibbonIcon) {
			...
			this.autoPasteToggleIconEl.toggleClass(
				"is-active",
				this.settings.autoPaste
			);
			this.registerDomEvent(
				this.autoPasteToggleIconEl,
				"contextmenu",
				(evt) => {
					if (menuShown) return;
	...
	private registerLibraryView() {
		// Register the new view
		this.registerView(
			LibraryView.VIEW_TYPE,
			(leaf: WorkspaceLeaf) => new LibraryView(this.app, this.plugin, leaf)
		);
	}
	...
	tryPatchUntilSuccess(
		patcher: (plugin: LibraryPlugin) => boolean,
	...
	private registerPDFEmbedCreator() {
		const originalPDFEmbedCreator =
			this.app.embedRegistry.embedByExtension["pdf"];

		this.register(() => {
			this.app.embedRegistry.unregisterExtension("pdf");
			this.app.embedRegistry.registerExtension(
				"pdf",
				originalPDFEmbedCreator
			);
		});
			...
			(ctx, file, subpath) => {
				...
				if (params.has("rect") && params.has("page")) {
					...
					const rect = params
						...
						.split(",")
						.map((n) => parseFloat(n));
					const width = params.has("width")
						? parseFloat(params.get("width")!)
						: undefined;
				...
				if (!embed) {
					embed = originalPDFEmbedCreator(
						ctx,
						file,
						subpath
					) as PDFEmbed;
					// @ts-ignore
					if (!this.classes.PDFEmbed)
						this.classes.PDFEmbed = embed.constructor;
						...
						this.registerDomEvent(
							...
							(evt) => {
								if (
									isTargetHTMLElement(evt, evt.target) &&
									evt.target.closest(
										'.pdf-embed[src*="#"] .pdf-viewer-container'
									)
								) {
									evt.preventDefault();
								}
				...
				if (params.has("color")) {
					embed.containerEl.dataset.highlightColor = params
						.get("color")!
						.toLowerCase();
				} else if (this.settings.defaultColor) {
					embed.containerEl.dataset.highlightColor =
						this.settings.defaultColor.toLowerCase();
				}
				return embed;
			}
		);
	...
	private registerGlobalVariable(
		name: string,
		value: any,
		...
		if (name in window) {
			if (throwError)
				throw new Error(
					`${this.manifest.name}: Global variable "${name}" already exists.`
				);
			else return;
		}
		// @ts-ignore
	...
	private registerEvents() {
		...
		this.registerEvent(
			this.app.workspace.on("layout-change", () => {
				for (const viewerEl of this.pdfViewerChildren.keys()) {
					if (!viewerEl?.isShown())
						this.pdfViewerChildren.delete(viewerEl);
				}
			})
		);
		...
		if (Platform.isDesktopApp) {
			this.registerEvent(
				this.app.workspace.on("active-leaf-change", (leaf) => {
					if (
						this.settings.syncWithDefaultApp &&
						leaf &&
						this.lib.isPDFView(leaf.view)
					) {
						...
						if (file) {
							...
							if (
								this.settings
									.focusObsidianAfterOpenPDFWithDefaultApp
							) {
								focusObsidian();
							}
						}
		...
		this.registerEvent(
			this.app.vault.on("rename", (file, oldPath) => {
				if (
					...
					this.settings.newFileTemplatePath === oldPath
				) {
		...
		this.registerEvent(
			this.app.vault.on("modify", async (file) => {
				if (
					file instanceof TFile &&
					file.path === this.settings.vimrcPath
				) {
		...
		this.registerEvent(
			this.app.workspace.on("editor-drop", (evt, editor, info) =>
				this.lib.dummyFileManager.createDummyFilesOnEditorDrop(
					evt,
					editor,
					info
				)
	...
	private registerHoverLinkSources() {
		...
		this.registerHoverLinkSource(
			PDFExternalLinkPostProcessor.HOVER_LINK_SOURCE_ID,
			{
				defaultMod: true,
		...
		this.registerHoverLinkSource(
			PDFOutlineItemPostProcessor.HOVER_LINK_SOURCE_ID,
			{
				defaultMod: true,
		...
		this.registerHoverLinkSource(
			PDFThumbnailItemPostProcessor.HOVER_LINK_SOURCE_ID,
			{
				defaultMod: true,
		...
		workspace.onLayoutReady(() => {
			...
			if (activeFile && activeFile.extension === "md") {
				...
				if (lastActiveMarkdownPath) {
					const lastActiveMarkdownFile = vault.getAbstractFileByPath(
			...
			this.registerEvent(
				vault.on("delete", (file) => {
					if (
						file instanceof TFile &&
						file === this.lastActiveMarkdownFile
					) {
	...
	obsidianProtocolHandler(params: ObsidianProtocolData) {
		if ("create-dummy" in params) {
			return this.lib.dummyFileManager.createDummyFilesFromObsidianUrl(
				params
			);
		}
	...
	requireModKeyForLinkHover(id = "pdf-plus") {
		// @ts-ignore
		return (
			this.app.internalPlugins.plugins["page-preview"].instance.overrides[
				id
			] ??
